...
SDK 目标为 netstandard2.0/netstandard2.1，CLI 需要 .NET 10。viewer-local CJK 仍依赖目标机器可用字体；PDF 自动加载器目前使用固定目录与 TTF 文件名，不能保证任意 OTF/TTC 或 DOCX FontDirectories 的替代字体可用。字体替代保留样式语义，但不保证字体造型与原 Word 相同。复杂文档、图片和页眉页脚需单独业务样例验收，详见 [转换契约](conversion-contracts.md)。
