...
internal readonly struct OfdFontStyleResult
{
    public readonly bool Bold;
    public readonly bool Italic;

    public OfdFontStyleResult(bool bold, bool italic)
    {
        Bold = bold;
        Italic = italic;
    }

    public void Deconstruct(out bool bold, out bool italic)
    {
        bold = Bold;
        italic = Italic;
    }
...
internal static class OfdFontStyle
{
    ...
    internal static OfdFontStyleResult Read(byte[] data)
    {
        if (data.Length < 12) return new OfdFontStyleResult(false, false);
        ...
        for (var index = 0; index < count && 12L + (index + 1L) * 16 <= data.Length; index++)
        {
            ...
            if (offset + 46 > data.Length) return new OfdFontStyleResult(false, false);
            var flags = (data[(int)offset + 44] << 8) | data[(int)offset + 45];
            return new OfdFontStyleResult((flags & 1) != 0, (flags & 2) != 0);
        }
        return new OfdFontStyleResult(false, false);
    }
}
